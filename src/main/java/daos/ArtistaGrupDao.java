...
public class ArtistaGrupDao {
    ...
    public Actuacio getArtistaGrup(int idArtista) {
        try {
            return jdbcTemplate.queryForObject("SELECT * FROM artistagrup WHERE idartista=?", new ActuacioRowMapper(), idArtista);
        } catch (EmptyResultDataAccessException e) {
            return new Actuacio();
        }
    }
